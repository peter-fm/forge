# Forge PR Blueprint Design — Implemented Notes

`pr-review.toml` is now generated by `forge init` alongside the default branching blueprints. This document remains as the design reference for that workflow.

When dogfooding these workflows in the `forge` repository itself, replace `main` with `master`. The generated prompts currently assume a `main` default branch, but this repo's default branch is `master`.
